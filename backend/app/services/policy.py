...
POLICY_PATTERNS = [
    ...
    r"\bhidden api key\b",
    r"\bapi keys?\b",
    # Expanded credential/token patterns to catch variations
    r"\baccess\s+tokens?\b",
    r"\bbearer\s+tokens?\b",
    r"\bauthenti(?:cation|zation)\s+tokens?\b",
    r"\bcredentials?\b",
    r"\b(?:pass|passwd|password)(?:s|words?)?\b",
    r"\b(?:auth|jwt|token)s?\b",
    r"\bsession\s+(?:id|key|token)\b",
    r"\b(?:oauth|oauth2|openid)\b",
    r"\b(?:client\s+)?(?:id|secret)\b",
    r"\bleaky?\s+(?:key|secret|credential|password)\b",
    r"\bleak\b",
    # Prompt-injection / adversarial patterns
    r"\bignore\s+(previous|prior|all|above|your)\b",
    r"\bbypass\s+(your|the|all|any)\b",
    r"\breveal\s+(internal|private|secret|hidden|confidential|key|password|credential|token)\b",
    r"\bdump\s+(all|the|secret|key|hidden|document|data|doc|credential|token)\b",
    r"\bsystem\s+prompt\b",
    r"\bprompt\s+injection\b",
    r"\bsalary\s+band\b",
    r"\bsalary\s+spreadsheet\b",
    r"\bhidden\s+(api|key|secret|document|data|credential|token)\b",
    r"\bexpose\s+(key|secret|credential|password|internal|token)\b",
    r"\bextract\s+(secret|hidden|private|confidential|credential|token)\b",
]
